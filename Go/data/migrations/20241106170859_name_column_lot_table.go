...
func init() {
	goose.AddMigrationContext(upNameColumnLotTable, downNameColumnLotTable)
}

func upNameColumnLotTable(ctx context.Context, tx *sql.Tx) error {
	...
	for _, lot := range lots {
		...
		_, err = tx.Exec(query, newLot.LotID, newLot.Latitude, newLot.Longitude, "", newLot.Address, newLot.Open, newLot.Close, newLot.Days, newLot.Decals, newLot.Occupancy, newLot.Capacity, newLot.Notes, newLot.Verified)
		if err != nil {
			return err
		}
	}
...
func downNameColumnLotTable(ctx context.Context, tx *sql.Tx) error {
